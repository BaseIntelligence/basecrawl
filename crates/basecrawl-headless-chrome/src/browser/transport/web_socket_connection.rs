...
impl DeadlineStream {
    fn new(inner: TcpStream, deadline: Option<Instant>, setup_phase: Arc<AtomicBool>) -> Self {
        Self {
            inner,
            deadline,
            setup_phase,
...
fn connect_tcp(
    ...
    Err(last_error.map_or_else(
        || anyhow::anyhow!("no addresses found for websocket host {host}"),
        anyhow::Error::from,
    ))
}
...
impl WebSocketConnection {
    pub fn new(
        ...
        let (connection, _) = Self::websocket_connection_with_root_cert(
            ws_url,
            root_cert.as_deref(),
            deadline,
