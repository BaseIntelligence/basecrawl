...
fn run(cli: Cli) -> Result<String, Error> {
    ...
    let difficulty = match cli.difficulty.as_deref() {
        None => None,
        Some(raw) => Some(crate::stealth::SiteDifficulty::parse(raw).ok_or_else(|| {
...
fn log_verbose_summary(proof: &crate::ScrapeProof) {
...
fn write_screenshot(proof: &crate::ScrapeProof, path: &std::path::Path) -> Result<(), Error> {
    let b64 = proof
        .result
        .formats_produced
        .get("screenshot")
...
/// Entry point for the `basecrawl` CLI binary (also used by the thin `basecrawl` install crate).
pub fn main() {
