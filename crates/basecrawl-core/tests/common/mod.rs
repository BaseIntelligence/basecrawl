...
/// Chromium `net::ERR_UNSAFE_PORT` denylist (subset from Chromium `net/base/port_util.cc`).
///
/// Ephemeral `bind(127.0.0.1:0)` occasionally lands on these ports (e.g. 6665–6669). Chrome
/// refuses document navigations to them, which flakes DPI/render integration tests that crawl
/// loopback fixtures. Soft/rustls clients ignore this list; only ports used as Chromium
/// navigation targets need to avoid it.
const CHROMIUM_RESTRICTED_PORTS: &[u16] = &[
...
/// True when Chromium refuse navigation to this TCP destination port.
pub fn is_chromium_restricted_port(port: u16) -> bool {
    CHROMIUM_RESTRICTED_PORTS.binary_search(&port).is_ok()
}

/// Bind a loopback TCP listener whose OS-chosen port is safe for Chromium navigation.
///
/// Retries ephemeral binds until the chosen port is outside Chromium's restricted list.
/// Fails closed after a bounded number of attempts so a pathological environment does not hang.
pub fn bind_chromium_safe_loopback() -> TcpListener {
    ...
    for _ in 0..MAX_ATTEMPTS {
        ...
        let port = listener
            ...
            .port();
        last_port = Some(port);
        if !is_chromium_restricted_port(port) {
            return listener;
        }
        // Drop and rebind: kernel almost always chooses another ephemeral port.
        drop(listener);
    }
    panic!(
        "could not bind a Chromium-safe loopback port after {MAX_ATTEMPTS} attempts (last port {last_port:?})"
    );
}
...
pub fn fixture_base() -> &'static str {
    ...
    BASE.get_or_init(|| {
        let listener = bind_chromium_safe_loopback();
